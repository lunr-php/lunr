...
class APNSDispatcher implements PushNotificationDispatcherInterface
{
    ...
    public function push()
    {
        ...
        try
        {
            $this->apns_message->addRecipient($this->endpoint);
        }
        catch(ApnsPHP_Message_Exception $e)
        {
        ...
        // Return response
        $response = new APNSResponse($this->logger, $this->endpoint, $invalid_endpoint, $errors);
     ...
     *
     * @param Array|String $endpoint The endpoint for the push
     *
     * @return APNSDispatcher $self Self reference
     */
    public function set_endpoint($endpoint)
    {
        $this->endpoint = is_array($endpoint) ? $endpoint[0] : $endpoint;
